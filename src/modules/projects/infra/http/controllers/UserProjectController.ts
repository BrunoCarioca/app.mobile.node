...
export class UserProjectController {
    public async addUserToProject(request: Request, response: Response): Promise<Response> {
        const { users, companyId } = request.body;
        const userId = Number(request.user.id);
        const { id } = request.params;

        const projectUsersRepository = new ProjectsUsersRepository();
        const usersCompanyRespository = new CompaniesToUsersRepository();
        const projectRespository = new ProjectRepository();

        const addUserProjectService = new AddUserProjectService(
            usersCompanyRespository,
            projectUsersRepository,
            projectRespository,
        );

        await addUserProjectService.execute({ userId, projectId: id, users, companyId });

        return response.status(200).json([]);
    }

    public async listUserInProject(request: Request, response: Response): Promise<Response> {
        const { id } = request.params;
        const userId = Number(request.user.id);

        const projectUsersRepository = new ProjectsUsersRepository();
        const listUserProjectService = new ListUserProjectService(projectUsersRepository);
        const users = await listUserProjectService.execute({ userId, projectId: id });

        return response.status(200).json(users);
    }

    public async removeUserToProject(request: Request, response: Response): Promise<Response> {
        const { users } = request.body;
        const userId = Number(request.user.id);
        const { id } = request.params;

        const projectUsersRepository = new ProjectsUsersRepository();
        const removeUsersInProjectService = new RemoveUsersInProjectService(projectUsersRepository);
        await removeUsersInProjectService.execute({ userId, projectId: id, users });
